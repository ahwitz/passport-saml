...
Strategy.prototype.authenticate = function (req, options) {
  ...
  // If we've received a buffer, convert it to a JSON-like object so that the default passport-saml code works
  if (req.body && (req.body instanceof Buffer)) {
    var str = "";
    for (i = 0; i < req.body.length; i++) {
      str += '%' + ('0' + req.body[i].toString(16)).slice(-2);
    }
  
    req.body = JSON.parse('{"' + decodeURI(decodeURIComponent(str)).replace(/"/g, '\\"').replace(/&/g, '","').replace(/=/g,'":"') + '"}');
    for (var param in req.body)
      req.body[param] = new Buffer(decodeURIComponent(req.body[param]), 'base64').toString('utf8');
  }
